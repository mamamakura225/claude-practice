...
## Firebase
piano-pet は dtask の Firebase プロジェクト `dtask-d08b6` を共有する（`pianopet` collection に保存）。認証なし・config直書き・`cloud.js` は動的importでロード（PWAのオフライン動作を壊さないため）。Firestoreルールは pianopet を許可済み。
