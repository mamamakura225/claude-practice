...
const BUDGETS_KIB = {
  ...
  // 込み）で 72.7→74.15 KiB（**LF正規化した実測**。ローカルWindows・core.autocrlf=true の
  // 作業ツリーは CRLF で 0.3〜0.4 KiB 水増しされる＝この数字を見て判断しないこと。
  // .gitattributes 無しでコミットは LF、CIもLFなので実際の判定はこちらが効く）。
  // 最小構成でも 0.15 KiB 超過・total は 93.55/94 でまだ余裕があるため js-entry だけ 75 へ。
  // #311〜#320: データ健全性修正群（下記 total のコメント参照）のガードで LF 実測 74.15→76.2。
  // 識別子は既出で gzip 済み・コメントは docs へ逃がし済みで圧縮の余地がないため 77 へ。
  'js-entry': 77,
  ...
  // 可読性を損なう（CLAUDE.md ④）。コメントは docs へ逃がしたうえで LF 実測 93.55→95.7 のため 96 へ。
